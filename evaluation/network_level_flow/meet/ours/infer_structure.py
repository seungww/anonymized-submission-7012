...
def infer_metric_all(model, dataset, device, logger,
    ...
    with torch.no_grad():
        for i in range(min(len(dataset), num_samples)):
            ...
            #w_dH, w_curH, w_dM, w_curM = 0.25, 0.25, 0.25, 0.25
...
def main():
    ...
    infer_metric_all(model, dataset, device, logger, num_samples=args.num_examples, display_start=args.display_start, display_end=args.display_end, th_final=1)
