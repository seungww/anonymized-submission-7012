...
def inferfloat(txt,valuescale,isLE=True):
    ...
    for i in range(max_k):
        ...
        if predictfloat(ys,LE=isLE):
            ...
            s = SIGMA([FIELD(intervals,annotation= endian + " Float " + "min("+str(min(fs))+") max(" +str(max(fs))+") mean(" +str(statistics.mean(fs))+") stdev(" +str(_safe_stdev(fs))+")",valuescale=valuescale)])
            sigmas.append(s)
